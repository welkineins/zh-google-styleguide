...
此外，列表初始化不允許整數類型的四捨五入，這可以用來避免一些類型上的編程失誤。
...
.. tip::
    ...
    訪問控制塊的宣告依次序是 ``public:``, ``protected:``, ``private:``, 每次縮進 1 個空格.
...
注意事項:
    ...
    - 關於宣告順序的規則請參考 :ref:`聲明順序 <declaration-order>` 一節.
...
基本原則是: 同一屏可以顯示的程式碼越多, 越容易理解程序的控制串流. 當然, 過於密集的代碼塊和過於疏鬆的代碼塊同樣難看, 取決於你的判斷. 但通常是垂直留白越少越好.
...
#. 前處理指令/命名空間不使用額外縮進, 類/結構體/列舉/函數/語句使用縮進;
#. 初始化用 ``=`` 還是 ``()`` 依個人喜好, 統一就好;
#. ``return`` 不要加 ``()``;
#. 水平/垂直留白不要濫用, 怎麼易讀怎麼來.
