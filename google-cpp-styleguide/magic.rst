...
定義：
    ...
    所有權是一種登記／管理動態內存和其它資源的技術。動態分配出的對象的所有主是一個對像或函數，後者負責確保當前者無用時就自動銷毀前者。所有權有時可以共享，那麼就由最後一個所有主來負責銷毀它。甚至也可以不用共享，在程式碼中直接把所有權傳遞給其它對象。
...
優點：
    ...
    * 傳遞所有權也比「借用」指針或引用來得簡單，畢竟它大大省去了兩個使用者一起協調對像生命週期的工作。
...
某些專案會指導你如何使用他們的項目工具運行 ``cpplint.py``. 如果你參與的項目沒有提供, 你可以單獨下載 `cpplint.py <http://github.com/google/styleguide/blob/gh-pages/cpplint/cpplint.py>`_.
